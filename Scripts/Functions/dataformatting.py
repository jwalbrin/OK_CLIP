...
def mod_fit_bars(data_mat, best_k_feats, targ_dims, y_label, y_lims):
    ...
    # Figure, axes set up
 	...
 	ax.set_xticks(np.arange(1,len(targ_dims)+1),labels = targ_dims, 
                                                     rotation = 90)
 	ax.set_ylabel(f"Model Fit ({y_label})")
 	ax.set_ylim(y_lims)
     
    # Plot data
 	ax.bar(np.arange(1,len(targ_dims)+1),
		np.mean(data_mat, axis = 0),
		yerr = (np.std(data_mat,axis = 0, ddof=1) 
		    / np.sqrt(data_mat.shape[0])))