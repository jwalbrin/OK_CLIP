""" 
kstrat_nested_regression_predict.py
Generate kstratified CV predictions
...
# --- User input
data_object_name = "kstrat_10_nested_object_clip-vit_eighty_tools.pkl"
...
def get_bkc_idx_nested(bkc_mat, fold, bks_idx, td_idx):
    """Get indices for the best k components across nested folds
    (sum-highest frequency across folds)"""
    bkc_mat_f = bkc_mat[fold, :, :, bks_idx, td_idx]
    bkc_mat_f[bkc_mat_f > 1] = 0
    bkc_mat_f = np.sum(bkc_mat_f, axis=0)
    bkc_idx = np.sort(np.flip(np.argsort(bkc_mat_f))[:bks])
    return bkc_idx
...
for td_idx, td in enumerate(targ_dims_flat):
    ...
    for bks_idx, bks in enumerate(best_k_sizes):
        for f in np.arange(n_fold):
            ...
            bkc_idx = get_bkc_idx_nested(data_object.bkc_mat, f, bks_idx, td_idx)
